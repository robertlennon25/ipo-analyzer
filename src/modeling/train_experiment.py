...
EXCLUDE_COLS: set[str] = {
    "ipo_year",
    # is_hot_ipo_year is a binary flag encoding 2020/2021 directly → regime-aware shortcut.
    # Excluded so models must learn market conditions from continuous features only.
    "is_hot_ipo_year",
    # total_proceeds_m is set to 100 as a placeholder for most IPOs (real data unavailable).
    # All derived normalized forms are also excluded since they encode the same bogus signal.
    "total_proceeds_m",
    "total_proceeds_m_year_z",
    "total_proceeds_m_year_pctile",
    "total_proceeds_m_roll360_z",
    "total_proceeds_m_roll360_pctile",
}
...
def _build_model_list(
    ...
    custom_params: optional per-model override dict loaded from tuned_params.json.
      Structure: { model_name: { param_name: value, ... } }
      Merges on top of HYPERPARAMS — only keys present in custom_params are overridden.
    """
    hp = copy.deepcopy(HYPERPARAMS)
    if custom_params:
        for model_name, params in custom_params.items():
            if model_name in hp:
                hp[model_name].update(params)
    return [
...
def load_all_feature_sets() -> tuple[dict[str, pd.DataFrame], dict[str, str]]:
    ...
    # PCA-compressed embeddings (optional — run pca_embeddings.py first)
    _load("pca_embeddings", PROCESSED_DIR / "pca_embeddings.csv")

    # Rolling-window regime features (leakage-free alternative to regime_normalized)
    # Generated by: python src/features/regime_normalized.py --rolling
    _load("regime_normalized_rolling",
          PROCESSED_DIR / "regime_normalized_rolling_features.csv")
...
def build_variants(
    ...
    if which in ("pca", "all"):
        # P1_text_pca: handcrafted NLP + PCA-compressed embeddings + proceeds
        # Replaces raw 384-dim embeddings with N principal components so that
        # Random Forest splits across many dimensions instead of 2-3 dominant ones.
        parts = []
        if "handcrafted" in feature_sets:
            parts.append(_clean(feature_sets["handcrafted"]))
        if "pca_embeddings" in feature_sets:
            parts.append(feature_sets["pca_embeddings"])   # already numeric, no _clean needed
        if "proceeds" in feature_sets:
            parts.append(_clean(feature_sets["proceeds"]))
        if parts:
            p1 = parts[0]
            for p in parts[1:]:
                p1 = _merge(p1, p, how="left")
            v["P1_text_pca"] = p1
        else:
            logger.warning("P1_text_pca skipped — run pca_embeddings.py first.")

        # P2_structured: identical to E2 (structured features, no change)
        parts = []
        if "multiples" in feature_sets:
            parts.append(_clean(feature_sets["multiples"]))
        if "market" in feature_sets:
        ...
        if parts:
            p2 = parts[0]
            for p in parts[1:]:
                p2 = _merge(p2, p, how="outer")
            v["P2_structured"] = p2

        # P3_combined_pca: PCA text + structured
        if "P1_text_pca" in v and "P2_structured" in v:
            v["P3_combined_pca"] = _merge(v["P1_text_pca"], v["P2_structured"], how="inner")

    if which in ("pca_v2", "all"):
        # P-series v2: regime-unaware.
        # Changes vs pca (v1):
        #   1. is_hot_ipo_year excluded globally via EXCLUDE_COLS (no 2020/2021 regime flag)
        #   2. P2/P3 use rolling 360-day normalization (_roll360_z/pctile) instead of
        #      calendar-year grouping — strictly causal, no forward leakage within a year

        # P1_v2_text_pca: identical to P1 (text-only, unaffected by regime change)
        parts = []
        if "handcrafted" in feature_sets:
            parts.append(_clean(feature_sets["handcrafted"]))
        if "pca_embeddings" in feature_sets:
            parts.append(feature_sets["pca_embeddings"])
        if "proceeds" in feature_sets:
            parts.append(_clean(feature_sets["proceeds"]))
        if parts:
            p1v2 = parts[0]
            for p in parts[1:]:
                p1v2 = _merge(p1v2, p, how="left")
            v["P1_v2_text_pca"] = p1v2
        else:
            logger.warning("P1_v2_text_pca skipped — run pca_embeddings.py first.")

        # P2_v2_structured: rolling regime normalization replaces calendar-year version
        if "regime_normalized_rolling" not in feature_sets:
            logger.warning(
                "P2_v2_structured skipped — regime_normalized_rolling_features.csv missing. "
                "Run: python src/features/regime_normalized.py --rolling"
            )
        else:
            parts = []
            for key in ("multiples", "market", "underwriter", "proceeds"):
                if key in feature_sets:
                    parts.append(_clean(feature_sets[key]))
            parts.append(_clean(feature_sets["regime_normalized_rolling"]))
            if parts:
                p2v2 = parts[0]
                for p in parts[1:]:
                    p2v2 = _merge(p2v2, p, how="outer")
                v["P2_v2_structured"] = p2v2

        # P3_v2_combined_pca: P1_v2 + P2_v2
        if "P1_v2_text_pca" in v and "P2_v2_structured" in v:
            v["P3_v2_combined_pca"] = _merge(
                v["P1_v2_text_pca"], v["P2_v2_structured"], how="inner"
            )
...
def train_for_target(
    ...
    hyperparams_override: loaded from tuned_params.json via --hyperparams.
      Structure: { variant_name: { model_name: { param: value } } }
      Per-variant params take priority; falls back to global HYPERPARAMS defaults.
    """
...
def run_experiment(
    ...
    # Supports two formats produced by tune_hyperparams.py:
    #   per-target: { target: { variant: { model: params } } }  ← new (multi-target tuning)
    #   flat:       { variant: { model: params } }               ← legacy (single-target tuning)
    hyperparams_override: dict | None = None   # always flat after normalisation
    hyperparams_by_target: dict | None = None  # populated when per-target format detected
    if hyperparams_path:
        hp_path = Path(hyperparams_path)
        if not hp_path.exists():
            raise FileNotFoundError(f"--hyperparams file not found: {hp_path}")
        raw = json.loads(hp_path.read_text())
        # Detect format: per-target keys are known target names; flat keys are variant names.
        known_targets = {"label_1w", "label_1m", "label_6m", "label_1y"}
        if raw and set(raw.keys()) <= known_targets:
            hyperparams_by_target = raw
            print(f"Hyperparams: {hp_path} (per-target format, "
                  f"targets: {list(raw.keys())})")
        else:
            hyperparams_override = raw
            print(f"Hyperparams: {hp_path} (flat format, "
                  f"{len(raw)} variants overridden)")
    ...
    for target in targets:
        ...
        print(f"{'#'*60}")
        # Per-target format takes priority; fall back to flat override or None
        effective_override = (
            (hyperparams_by_target or {}).get(target)
            or hyperparams_override
        )
        result = train_for_target(target, returns_df, variants, models_dir,
                                  hyperparams_override=effective_override)
        if result:
    ...
    run_envelope = {
        ...
        "hyperparams_override":  hyperparams_override,   # None if defaults used
...
if __name__ == "__main__":
    ...
    parser.add_argument("--variants", default="enhanced",
                        ...
                        help="Which variant family to train. "
                             "'pca' trains P1/P2/P3 (calendar-year regime). "
                             "'pca_v2' trains P1_v2/P2_v2/P3_v2 (rolling 360d, regime-unaware). "
                             "'all' trains all families.")
    ...
    parser.add_argument("--hyperparams", default=None, metavar="PATH",
                        help="Path to tuned_params.json from tune_hyperparams.py. "
                             "Overrides default HYPERPARAMS per variant × model.")
